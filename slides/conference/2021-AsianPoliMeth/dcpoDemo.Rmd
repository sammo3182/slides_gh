...
Support .navy[&uarr;] &rarr; democracy .orange[&#8675;] .small[(e.g., Fails and Pierce 2010)]
...
Democracy .navy[&uarr;] &rarr; support .red[&darr;] .small[(e.g., Foa and Mounk 2017)]  
???
...
long-time democracies are seeing declining support, so we should have a down arrow on support for them.
...
???
Different respondents have different concepts of democracy. Swarming Capital becomes defending democracy.  
...
$$Pr({\eta}_{ktqr} = 1) = logist^{-1}[\frac{\bar{\theta}'_{kt} - {(\beta_{qr} + \color{red}{\delta_{kq}})}}{\sqrt{\alpha^2_q + (1.7\times \sigma_{kt})^2}}]$$

$$y_{ktqr}\sim  BetaBinomial ({\eta}_{ktqr},a_{ktqr},b_{ktqr})$$
...
.pull-right[
.red[&delta;: Country-specific item bias] 
.red[&sigma;: polarization in public opinion]]

???

Dispersion: magnitude of the measurement error 
$$a_{ktqr} = {\phi}{\eta}_{ktqr}, b_{ktqr} = {\phi}(1-{\eta}_{ktqr}) $$
A beta-binomial link inlcudes an additional dispersionparameter capturing the uncertainty beyond the mere sampling error.
...
.center[<img src="images/validationTest.png" height = 380 />]

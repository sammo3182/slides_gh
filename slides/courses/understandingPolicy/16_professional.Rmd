...
```{r ranNm-generator, echo = TRUE, eval=FALSE}
# The file downloaded from the Web Learning

studentNM <- readxl::read_excel(here("slides",
                             "courses",
                             "understandingPolicy",
                             'student.xls')) %>% 
  select(id = `姓名`) %>% 
...
day1 <- rsocialroulette(studentNM, m = 2)
past <- list(day1) %>% setNames(Sys.Date())

day2 <- studentNM %>% 
  mutate(date = Sys.Date() + 1) %>% 
  rsocialroulette(past_partitions = past, m = 2)

df_present <- partitions_to_pairs(list(day1, day2))
write.csv(df_present, file = "D:/MEGAsync/00_Methods/Courses/Nonmethod Courses/Teaching/01_courses/02_Analysis_of_Public_Policy/students/presentationList.csv", 
            fileEncoding = "UTF8")
```

```{r speech10, echo = FALSE}

...
![](http://103.238.162.29:10002/webdav/sammo3182/figure/mr_fit.gif){.r-stretch fig-align="center"}
...
::: {.column width="50%"}
![](http://103.238.162.29:10002/webdav/sammo3182/figure/mr_2x.png){.fragment fig-align="center" height=500}
:::
...
:::{.fragment}
:::{.callout-important}
[**Interpretation**: Every unit change in X<sub>1</sub> leads to &beta;<sub>1</sub> changes in Y [on average]{.red}, [ceteris paribus/holding everything else constant]{.red}.]{.large}
:::
:::
...
# Goodness of Fit
## R<sup>2</sup>: Multiple Coefficient of Determination{.smaller}
...
![](http://103.238.162.29:10002/webdav/sammo3182/figure/mr_bad4ya.jpg){.r-stretch fig-align="center"}


## Why Is R<sup>2</sup> Bad? 

> Can be very [low]{.red} for a correct model

$$R^2 = \frac{SSR}{SST} = \frac{\sum(\hat Y_i  - \bar Y)^2}{SST} = \frac{\sum[(Y_i - \epsilon_i) - \bar Y]^2}{SST}.$$

When the residual (thus &sigma;, estimated by &epsilon;<sub>i</sub> in sample estimations) is large enough, R<sup>2</sup> could approach a very low score towards zero.
...
> Can be very [high]{.red} for a misspecified model
...
## Why Is R<sup>2</sup> Bad? {auto-animate=true}

> Can be very [high]{.red} for a redundant model
...
## Is Adj. R<sup>2</sup> still bad? {auto-animate=true}
...
Let X<sub>0</sub> be the values at which we want to predict. 
...
ggplot(mtcars, aes(x = mpg, y = wt)) +
    ...
    ylab(bquote(hat(Y[0]))) +
    xlab(bquote(hat(X[0]))) +
    labs(caption = "The ribbon represents the standard error. Why is it wider at the two ends?") +
    theme(
        plot.title = element_text(size = 28),
        axis.text = element_text(size = 20),
        axis.title = element_text(size = 20, face = "bold")
...
var(\hat Y_0|X_0) =& \sigma^2[\color{red}{1} + \frac{1}{n} + \frac{(X_0 - \bar X)^2}{\sum(X_i - \bar X)^2}].
\end{align}
...
## Hypothesis Test: Restricted Model{.smaller}
...
## Latin Terms <img src = "http://103.238.162.29:10002/webdav/sammo3182/figure/ci_fsmrof.png" height = 30>
...
![](http://103.238.162.29:10002/webdav/sammo3182/figure/mr_mindmap.png){.r-stretch}

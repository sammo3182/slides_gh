...
For every collection of time periods ( $X_t, X_{t_2}, X_{t_3},\cdots$ where $t < t_2 < t_3,\cdots$ ), their distributions are the same as $X_t + h, X_{t_2} + h, X_{t_3} + h,\cdots$, i.e., the sam joint distribution, or saying the error terms are the same for every period.
...
Then, a moving average process that is covariance stationary is weakly depend if cov(X<sub>t</sub>X<sub>t + n</sub>)&rarr;0 as h&rarr;&infin;.
...
var(u_t|X) =& \sigma_e^2 + \rho^2\sigma_e^2 + \rho^4\sigma^2 +\cdots...\\
           =& \sigma_e^2\sum^T_{i = 1}\rho^{2(i - 1)}\\
...
Structural break at time t<sup>*</sub> in the relationship between X and Y
...
In this case, the model is called *underidentified* model.

Consequence : .red[biased, inconsistent] estimate of &beta;.
...
We believe the full (true) model is $Y = \beta_0 + \beta_1X_1 + \beta_2X_2 + u$, and if one estimates a constrained model $Y = \beta_0 + \beta_1X_1 + u'$, X<sub>2</sub> is missed. 
...
When do people go back to school after working already?

DV : Income
IV : Likelihood of backing to school
...
1. Rainfall growth (percentage change in rainfall from the previous year) (Miguel, Satyanath, and Sergenti 2004)
...
\hat{\boldsymbol{\beta}}_{2SLS} =& (\boldsymbol{X'P_ZX})^{-1}(\boldsymbol{X'P_ZY})\\
                                =& (\boldsymbol{Z'\hat X})^{-1}(\boldsymbol{Z'Y})
\end{align}
...
\Rightarrow\beta_1 =& \frac{cov(Y_i, Z_i)}{\beta_1cov(X_i, Z_i)} = \frac{\sum(Y_i - \bar Y)(Z_i - \bar Z)}{\sum(X_i - \bar X)(Z_i - \bar Z)},\\
       var(\hat\beta_1) =& \frac{\sigma^2}{n\sigma_X^2\rho_{XY}^2} \geq \frac{\sigma^2}{n\sigma_X^2\rho_{XX}^2} = \frac{\sigma^2}{\sum(X_i - \bar X)^2}
\end{align}
...
This will produce a unbiased result but not exactly &beta;<sub>1</sub>.

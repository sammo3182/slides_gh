...
.center[P(M|D) = P(M|D<sup>Observed</sup>)]

### Non-ignorable (NI/MNAR)

.center[P(M|D) &ne; P(M|D<sup>Observed</sup>)]
...
|------|---------------|---------------------|-----------------|
| MCAR | Unbiased      | Unbiased/consistent | .navy[Inefficient]     |
| MAR  | Biased        | Unbiased/consistent | .navy[Inefficient]     |
| NI   | Biased        | .red[Biased]              | --     |
...
&rArr; misspecification of the model does not affect MI.
...
1. Model is .navy[conditional on X] + .navy[correctly specified]
    + Listwise would not affect the analysis.
    
--

1. .navy[Large] data
    + Listwise is [trivial](https://statisticalhorizons.com/listwise-deletion-its-not-evil).
    
--

1. Missingness in X is .navy[not a function of Y], and there is no unobserved omitted values that affects Y.

--

1. There is .red[NI] missingness in X
    + EMs incorrect
...
+ .red[Generalized] linear regression is not .navy[general] linear regression
+ Using when the very first assumption of OLS is violated---Y is no longer continuous.
...
When G() = &Lambda;(X), logit (.orange[log]istic un.orange[it])

When G() = &Phi;(X), progit (.orange[pro]bability un.orange[it])
...
## Estimation<sup><img src="images/fsmrof.png" height="20"/></sup>

We cannot minimize the least square as in OLS, since Y<sup>*</sup> is not observed (unable to calculate $(Y - \bar Y)^2$ ). So, one has to use the likelihood approach to inference.
...
$\hat\theta$ is the .navy[maximum likelihood estimate] of &theta; from among all possible values of &theta;<sup>*</sup>

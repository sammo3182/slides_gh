...
# CJK-capable font for plotting Chinese labels: pick one available on the render OS.
# "WenQuanYi Micro Hei" ships with Linux; Windows has "Microsoft YaHei" instead.
...
# Make the CJK font the default for every ggplot so Chinese labels render everywhere
theme_set(theme_gray(base_family = cjk_font))
```
...
- What is the [population]{.red} of a language? 
    - One person's words? A nation's? All of humanity's? 
- Language is open and ever-generating with no clear boundary [@Biber1993]. 
    - Size alone guarantees [no representativeness]{.red}.
...
- Meaning is [nested]{.red} from character to word to sentence to document. 
...
[**Statistical**]{.red} regularities the naked eye cannot see.
...
- **Function words** --- pronouns, prepositions, conjunctions:
    - Used almost *unconsciously*, yet betray attention, emotion, and social relations [@Pennebaker2017]. 

::: {.notes}

Example: Heavy use of "I" tracks depression; the emotional tone of Justices' words in oral argument even predicts their votes [@BlackEtAl2011a; @Liu2022].
...
|  | Content analysis | Discourse analysis | [Computational]{.green} |
|--|--|--|--|

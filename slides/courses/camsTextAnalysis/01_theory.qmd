...
if (!requireNamespace("jiebaR", quietly = TRUE)) {
  install.packages(
    c("https://cran.r-project.org/src/contrib/Archive/jiebaRD/jiebaRD_0.1.tar.gz",
...
p_load(
  icons,
  tidyverse,
  rvest, stringr, broom, jiebaR, tidytext
)
...
Long before spreadsheets, humanity stored its record in [words]{.red}.
...
> The usual answer---"because we can handle more data"---misses the point.
